...
def vSIRres(S, I , BETA, t):
    Sprime = lambda t: jax.grad(lambda t: jnp.sum(S(t)))(t)[:, 0]
    Iprime = lambda t: jax.grad(lambda t: jnp.sum(I(t)))(t)[:, 0]
    Sres = Sprime(t) + jax.nn.sigmoid(BETA(t))*S(t)*I(t)
    Ires = Iprime(t) - jax.nn.sigmoid(BETA(t))*S(t)*I(t) + gamma(t)*I(t)
    return Sres**2 + Ires**2
...
par_sa = {}
key_sa = jax.random.randint(jax.random.PRNGKey(0), (2,), 1, 1000000)
par_sa.update({'ws': 1*jax.random.uniform(key = jax.random.PRNGKey(key_sa[0]), shape = (ns, 2))})
par_sa.update({'wr': 1*jax.random.uniform(key = jax.random.PRNGKey(key_sa[1]), shape = (nc, 1))})
params.update({'sa' : par_sa})
...
@jax.jit
def reluforward(x, params, activation = jax.nn.relu):
    *hidden,output = params
    for layer in hidden:
...
def loss_func(params, Ts, Tc, sol, w=1):
    ...
    #loss += w*jnp.mean(MSE(vSIRres(lambda t: (forward(t, params['S'])[:, 0]), lambda t: (forward(t, params['I'])[:, 0]), lambda t: (forward(t, params['beta'])[:, 0]), Tc), 0))
    #loss += jnp.mean(MSE(forward(Ts, params['S']).reshape(ns,), sol[:, 0]))
    #loss += jnp.mean(MSE(forward(Ts, params['I']).reshape(ns,), sol[:, 1]))
    loss += w * jnp.mean(MSE_SA(vSIRres(lambda t: (forward(t, params['S'])[:, 0]), lambda t: (forward(t, params['I'])[:, 0]), lambda t: (forward(t, params['beta'])[:, 0]), Tc), 0, params['sa']['wr']))
    loss += jnp.mean(MSE_SA(forward(Ts, params['S']).reshape(ns,), sol[:, 0], params['sa']['ws'][:, 0]))
    loss += jnp.mean(MSE_SA(forward(Ts, params['I']).reshape(ns,), sol[:, 1], params['sa']['ws'][:, 1]))
    return loss
...
@jax.jit 
def update(opt_state, params, Ts, Tc, sol, w = 1):
    grads = grad_loss(params, Ts, Tc, sol, w)
    for w in grads['sa']:
        grads['sa'][w] = - grads['sa'][w]
    updates, opt_state = optimizer.update(grads, opt_state)
    params = optax.apply_updates(params, updates)
    return opt_state, params
plt.plot(Ts, jax.nn.sigmoid(forward(Ts, params['beta'])), 'g')
...
for e in range(epochs):
  ...
  if e % 1000 == 0:
      ...
      print(l)
plt.plot(Ts, sol)
plt.plot(Ts, forward(Ts, params['S']), 'r')
plt.plot(Ts, forward(Ts, params['I']), 'b')
plt.plot(Ts, list(map(beta, Ts)), 'c')
plt.plot(Ts, jax.nn.sigmoid(forward(Ts, params['beta'])), 'k')
