...
params = {}
widths = {'S': [1] + 4*[16] + [1], 'I': [1] + 4*[16] + [1], 'beta': [1] + 8*[100] + [1]}
...
for e in range(epochs):
    opt_state,params = update(opt_state,params, Ts, Tc, sol, 0)    
    print(e)    
alpha = (jnp.mean(MSE(forward(Ts, params['S']).reshape(ns,), sol[:, 0])) + jnp.mean(MSE(forward(Ts, params['I']).reshape(ns,), sol[:, 1])))/jnp.mean(MSE(vSIRres(lambda t: (forward(t, params['S'])[:, 0]), lambda t: (forward(t, params['I'])[:, 0]), lambda t: (forward(t, params['beta'])[:, 0]), Tc), 0))
print(alpha)
for phase in range(phases-1):
    for e in range(epochs):
        opt_state,params = update(opt_state,params, Ts, Tc, sol, alpha*10**(-2-phase))    
        print(e)    
plt.plot(Ts, sol)
