# -*- coding: utf-8 -*-
"""
Created on Sun Sep 29 14:58:50 2024
...
epochs = 1000000#0#1000000#200000#100000
sigma = 0
...
def vSIRres(S, I , BETA, GAMMA, t):
    Sprime = lambda t: jax.grad(lambda t: jnp.sum(S(t)))(t)[:, 0]
    Iprime = lambda t: jax.grad(lambda t: jnp.sum(I(t)))(t)[:, 0]
    Sres = Sprime(t) + jax.nn.sigmoid(BETA(t))*S(t)*I(t)
    Ires = Iprime(t) - jax.nn.sigmoid(BETA(t))*S(t)*I(t) + jax.nn.sigmoid(GAMMA(t))*I(t)
    return Sres**2 + Ires**2
...
params = {}
widths = {'S': [1] + 4*[16] + [1], 'I': [1] + 4*[16] + [1], 'beta': [1] + 4*[16] + [1], 'gamma': [1] + 4*[16] + [1]} #8*100
...
#    loss += w*jnp.mean(MSE(vSIRres(lambda t: (forward(t, params['S'])[:, 0]), lambda t: (forward(t, params['I'])[:, 0]), lambda t: (forward(t, params['beta'])[:, 0]), lambda t: (forward(t, params['gamma'])[:, 0]), Tc), 0))
...
def loss_func(params, Ts, Tc, sol, w=1, epsilon = 1):
    loss = 0
    residue = vSIRres(lambda t: (forward(t, params['S'])[:, 0]), lambda t: (forward(t, params['I'])[:, 0]), lambda t: (forward(t, params['beta'])[:, 0]), lambda t: (forward(t, params['gamma'])[:, 0]), Tc)
...
@jax.jit
def update(opt_state, params, Ts, Tc, sol, w = 1, epsilon = 1):
    grads = grad_loss(params, Ts, Tc, sol, w, epsilon)
...
for e in range(epochs):
  ...
  if e % 1000 == 0:
      l2_test = (L2error(forward(Ts, params['S']), sol[:, 0])).tolist()# + L2error(forward(Ts, params['I']), sol[:, 1])).tolist()
...
plt.plot(Ts, list(map(gamma, Ts)), 'y', label=r'$\gamma$')
plt.plot(Ts, jax.nn.sigmoid(forward(Ts, params['beta'])), 'k', label=r'$\mathscr{NN}_\beta$')
plt.plot(Ts, jax.nn.sigmoid(forward(Ts, params['gamma'])), 'g', label=r'$\mathscr{NN}_\gamma$')
plt.title(str(epochs) + ' ' + str(nc) + str(widths['gamma'][1]))
plt.legend()
